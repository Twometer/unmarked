...
function processList(buf, node, ordered) {
    let num = 0
    for (let child of node.childNodes) {
        if (child.nodeName == 'LI') {
            if (ordered) buf.append(`${++num}. `)
            else buf.append('- ')
            processChildren(buf, child)
            buf.append('\n')
        }
    }
}

function processChildren(buf, node) {
    for (let child of node.childNodes) {
        processNode(buf, child)
    }
}

function processNode(buf, node) {
    ...
    switch (node.nodeName) {
        case '#text':
            if (isWhitespaceSignificant || !isStringBlank(node.data)) buf.append(node.data)
            break
        case 'EM':
            buf.append('_')
        ...
        case 'HR':
            ...
            break
        case 'UL':
            processList(buf, node, false)
            break
        case 'OL':
            processList(buf, node, true)
            break
        case 'INPUT':
